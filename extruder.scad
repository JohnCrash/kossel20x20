...
module extruder() {
  rotate([90, 0, 0]) difference() {
    union() {
      ...
      //clamp
      translate([20, 0, 31]) cube([13, 20, 18]);
...
/*	    translate([16,20,21]) rotate([90,0,0]){
    ...
    translate([16,20-2,21]) rotate([90,0,0]) cylinder (h=5.6, r=8.5);
    ...
    difference() {
      union() {
       translate([31+2.4,9.5+4,21]) rotate([90,0,0]) cylinder (h=7, r=8.5);
       translate([31+2.4-5,9.5-5.25+4-1.8,21-8.25-1]) cube([20, 7, 18.5]);
        //opening between bearing and pulley
   #     translate([20,9.5-5.25+2.2,21-8.25+3.25-2]) cube([10, 7, 14]);
      }
      //removable supports
      for (z = [15:3:27]) {
    ...
    //clamp slit
    translate([25+2,-1,12]) cube([2, 22, 40]);
	#translate([25+2+1,21,12]) rotate([90,0,0])cylinder(r=1,h=22,$fn=12);
    //clamp nut
    translate([10.5,12+2,38+6.5]) rotate([0,90,0])
      cylinder(h=11, r=m3_nut_radius, $fn=6);
    //clamp screw hole
    translate([15,12+2,38+6.5]) rotate([0,90,0])
      cylinder(h=20, r=m3_wide_radius, $fn=12);
  }
}

extruder();
translate([16,-21,7])rotate([0,0,180])extruder_wheel(r=8);